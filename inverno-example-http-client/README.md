...
The HTTP client can also be configured with a keystore and a truststore to enable Mutual TLS authentication ([mTLS][mTLS]), the example application provides client keystore and truststore under `src/main/resources` matching [HTTP server example][inverno-example-http-server] certificate, they can be specified on the command line:

```plaintext
$ mvn clean inverno:run -Dinverno.run.arguments='--io.inverno.example.app_http_client.appConfiguration.http_client.tls_key_store=\"module:/clientkeystore.p12\" --io.inverno.example.app_http_client.appConfiguration.http_client.tls_key_store_type=\"PKCS12\" --io.inverno.example.app_http_client.appConfiguration.http_client.tls_key_store_password=\"password\" --io.inverno.example.app_http_client.appConfiguration.http_client.tls_trust_store=\"module:/clienttruststore.p12\" --io.inverno.example.app_http_client.appConfiguration.http_client.tls_trust_store_type=\"PKCS12\" --io.inverno.example.app_http_client.appConfiguration.http_client.tls_trust_store_password=\"password\"'
...
```
