...
/**
 * <p>
 * Starts an HTTP/2 server with TLS and HTTP compression using custom root and error handlers.
 * </p>
 * 
...
public class Main {
	...
	public static void main(String[] args) {
		// Starts the server
		Application.run(new App_http_server.Builder()
			// Setups the server
			.setAppConfiguration(
				AppConfigurationLoader.load(configuration -> configuration
					.http_server(server -> server
						...
						.server_port(8443)
						.tls_enabled(true)
						.tls_key_store(URI.create("module://io.inverno.example.app_http_server/serverkeystore.p12"))
						.tls_key_store_type("pkcs12")
						.tls_key_store_password("password")
						// mTLS
						.tls_client_auth(HttpServerConfiguration.ClientAuth.REQUESTED)
						.tls_trust_store(URI.create("module://io.inverno.example.app_http_server/servertruststore.p12"))
			...
			.setAppController(ServerController.from(
				exchange -> {
