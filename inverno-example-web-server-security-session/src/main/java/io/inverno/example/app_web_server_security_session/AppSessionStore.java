 ...
 */
@Wrapper @Bean
public class AppSessionStore implements Supplier<BasicSessionStore<AppAuthSessionData>> {
	...
	@Override
	public BasicSessionStore<AppAuthSessionData> get() {
		if(this.configuration.useRedisSessionStore()) {
			LOGGER.info("Using RedisSessionStore");
			return RedisBasicSessionStore.builder(this.redisClient, this.mapper, AppAuthSessionData.class).build();
		}
		else {
			LOGGER.info("Using InMemorySessionStore");
			return InMemoryBasicSessionStore.<AppAuthSessionData>builder(this.reactor)
				.build();
		}
	}
}
